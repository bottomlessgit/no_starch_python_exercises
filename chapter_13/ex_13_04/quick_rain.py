...
def createraindrop(qr_s, screen, drop_num, row_num):
    ...
    new_raindrop.rect.x = drop_num * (qr_s.r_width + qr_s.x_dist)
    new_raindrop.rect.y = qr_s.replacement_height + row_num * (qr_s.r_height + qr_s.y_dist)
...
def update_raindrops(qr_s, raindrops):
    ...
    for raindrop in raindrops:
        if raindrop.past_bottom():
            raindrops.add(raindrop.replace())
