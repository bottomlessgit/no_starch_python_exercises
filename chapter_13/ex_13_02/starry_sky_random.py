...
def create_stars(screen, ss_settings, stars):
    ...
    for row in range(ss_settings.num_rows):
        for star_num in range(ss_settings.num_stars_in_row):
            # Create random displacements
            rand_x_displacement = int(.5 * randint(-ss_settings.x_spacing, ss_settings.x_spacing))
            rand_y_displacement = int(.5 * randint(-ss_settings.y_spacing, ss_settings.y_spacing))
